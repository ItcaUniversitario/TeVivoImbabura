...
export function mostrarModalCasilla(casillaIndex) {
    ...
    switch (casillaData.tipo) {
        ...
        case 'boleto_pregunta':
            ...
            // 🔥 NUEVO: Creamos un historial en el gameState si no existe
            if (!gameState.historialTren) gameState.historialTren = [];

            // 🔥 NUEVO: Filtramos las preguntas para usar solo las que NO han salido
            let preguntasSobrantes = preguntasDisponibles.filter(p => !gameState.historialTren.includes(p.pregunta));

            // Si ya salieron todas las preguntas, reiniciamos el historial para volver a empezar
            if (preguntasSobrantes.length === 0) {
                gameState.historialTren = [];
                preguntasSobrantes = preguntasDisponibles;
            }

            // Elegimos una aleatoria de las que SOBRAN
            const preguntaActual = preguntasSobrantes[Math.floor(Math.random() * preguntasSobrantes.length)];

            // Guardamos la pregunta elegida en el historial para que no vuelva a salir pronto
            gameState.historialTren.push(preguntaActual.pregunta);
            ...
            opcionesMezcladas.forEach((opcionObj) => {
                ...
                btnOpt.innerText = opcionObj.texto; // Usamos el texto de la opción mezclada
                ...
                btnOpt.onclick = () => {
                    ...
                    // ✅ VALIDACIÓN POR ÍNDICE ORIGINAL DE LA BASE DE DATOS
                    if (opcionObj.indiceOriginal === preguntaActual.correcta) {
                        // ==========================================
